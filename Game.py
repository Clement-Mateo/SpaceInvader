...
# TODO animations
# TODO bonus
# TODO nouveaux ennemis et nouveau visuel des ennemis
...
class Enemy(pygame.sprite.Sprite):
    ...
    def __init__(self, image, screen, rect, isSound):
    ...
    def update(self):
        ...
        if (self.delayShot == 0):
            ...
            self.missiles.append(EnemyMissile(
                8, pygame.image.load("img/shot.gif").convert()))
...
class EnemyHorisontal(Enemy):
    # Appel du constructeur de la fonction (a l'appel de la classe)
    def __init__(self, image, screen, rect, isSound):
        super().__init__(image, screen, rect, isSound)
...
class Game(object):
    ...
    def invoqueEnnemies(self):
        if self.score < 6:
            self.enemies.append(Enemy(pygame.image.load(
                "img/alien.gif").convert(), self.screen, {"x": 0, "y": 0}, self.sound))
            self.enemies.append(Enemy(pygame.image.load(
                "img/alien.gif").convert(), self.screen, {"x": 200, "y": 0}, self.sound))
            self.enemies.append(Enemy(pygame.image.load(
                "img/alien.gif").convert(), self.screen, {"x": 400, "y": 0}, self.sound))
        elif self.score < 12:
            self.enemies.append(EnemyHorisontal(pygame.image.load(
                "img/alien.gif").convert(), self.screen, {"x": 0, "y": 0}, self.sound))
            self.enemies.append(EnemyHorisontal(pygame.image.load(
                "img/alien.gif").convert(), self.screen, {"x": 150, "y": 0}, self.sound))
            self.enemies.append(EnemyHorisontal(pygame.image.load(
                "img/alien.gif").convert(), self.screen, {"x": 300, "y": 0}, self.sound))
            self.enemies.append(EnemyHorisontal(pygame.image.load(
                "img/alien.gif").convert(), self.screen, {"x": 450, "y": 0}, self.sound))
            self.enemies.append(EnemyHorisontal(pygame.image.load(
                "img/alien.gif").convert(), self.screen, {"x": 600, "y": 0}, self.sound))
        elif self.score < 25:
            self.enemies.append(Enemy(pygame.image.load(
                "img/alien.gif").convert(), self.screen, {"x": 0, "y": 0}, self.sound))
            self.enemies.append(Enemy(pygame.image.load(
                "img/alien.gif").convert(), self.screen, {"x": 100, "y": 50}, self.sound))
            self.enemies.append(Enemy(pygame.image.load(
                "img/alien.gif").convert(), self.screen, {"x": 200, "y": 0}, self.sound))
            self.enemies.append(Enemy(pygame.image.load(
                "img/alien.gif").convert(), self.screen, {"x": 300, "y": 50}, self.sound))
            self.enemies.append(Enemy(pygame.image.load(
                "img/alien.gif").convert(), self.screen, {"x": 400, "y": 0}, self.sound))
            self.enemies.append(Enemy(pygame.image.load(
                "img/alien.gif").convert(), self.screen, {"x": 500, "y": 50}, self.sound))
    ...
    def draw_game_screen(self):
        ...
        if(self.ship.invincibility > 0):
            if(self.ship.invincibility % 5):
                image_invincible = pygame.image.load("img/ship.gif").convert()
    ...
    def setup(self):
        ...
        self.ship = Ship(pygame.image.load(
            "img/ship.gif").convert(), self.screen, self.sound)
        self.ship.rect.y = 400 - 25
        self.fond = pygame.image.load("img/earth.png").convert()
